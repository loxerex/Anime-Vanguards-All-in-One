...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Shibuya_Aftermath.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Shibuya_Aftermath",offset,no_restart=True)
        time.sleep(1)
        fd.click(29+offset[0],454+offset[1],right_click=True)
        time.sleep(4)
        fd.click(177+offset[0],419+offset[1],right_click=True)
    fd.start(offset)
    time.sleep(6)
