...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Kuinshi_Palace.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Kuinshi_Palace",offset,no_restart=True)
        time.sleep(1)
        fd.click(45+offset[0],242+offset[1],right_click=True)
    fd.start(offset)
    time.sleep(6)
