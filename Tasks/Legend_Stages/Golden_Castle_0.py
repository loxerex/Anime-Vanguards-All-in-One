...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Golden_Castle.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Golden_Castle",offset,no_restart=True,just_camera=True)
        time.sleep(1)
        fd.click(411+offset[0],76+offset[1],right_click=True)
    fd.start(offset)
    time.sleep(6)
