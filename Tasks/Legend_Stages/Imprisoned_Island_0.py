...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Imprisoned_Island.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Imprisoned_Island",offset,no_restart=True)
        time.sleep(1)
    fd.start(offset)
    time.sleep(6)
