...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Land_Of_The_Gods.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Land_Of_The_Gods",offset,no_restart=True,just_camera=True)
        time.sleep(1)
    fd.start(offset)
    time.sleep(6)
