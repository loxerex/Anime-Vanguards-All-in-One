...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Legend_Stages", "Sand_Village.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Sand_Village",offset,no_restart=True,zoom=True)
        time.sleep(1)
        fd.click(695+offset[0],482+offset[1],right_click=True)
        time.sleep(4)
    fd.start(offset)
    time.sleep(6)
