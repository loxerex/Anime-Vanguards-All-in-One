...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Story", "Martial_Island.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Martial_Island",offset,True)
        time.sleep(1)
        fd.click(28+offset[0],398+offset[1],right_click=True)
        time.sleep(5)
    while True:
        match_end = False
        win = False
