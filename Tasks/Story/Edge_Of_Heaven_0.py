...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Story", "Edge_Of_Heaven.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Edge_Of_Heaven",offset,True)
        time.sleep(1)
        fd.click(410+offset[0],619+offset[1],right_click=True)
        time.sleep(4)
        fd.click(344+offset[0],495+offset[1],right_click=True)
        time.sleep(4)
    while True:
