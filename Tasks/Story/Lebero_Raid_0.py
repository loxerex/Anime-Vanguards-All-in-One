...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Story", "Lebero_Raid.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Lebero_Raid",offset,True)
        time.sleep(1)
        fd.click(785+offset[0],591+offset[1],right_click=True)
        time.sleep(4)
        fd.click(239+offset[0],620+offset[1],right_click=True)
        time.sleep(4)
        fd.click(418+offset[0],619+offset[1],right_click=True)
        time.sleep(4)
    while True:
        match_end = False
        win = False
