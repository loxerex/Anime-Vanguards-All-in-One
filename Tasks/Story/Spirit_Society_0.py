...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Story", "Spirit_Society.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Spirit_Society",offset)
        time.sleep(1)
        fd.click(340+offset[0],79+offset[1],right_click=True)
        time.sleep(5)
        fd.click(70+offset[0],110+offset[1],right_click=True)
        time.sleep(5)
    while True:
        match_end = False
        win = False
