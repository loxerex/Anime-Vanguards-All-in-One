...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Boss_Rush", "Saber.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    First_Match = True
    AINZ_SPELL = False
    while not fd.does_exist("SaberBossRush.png",confidence=0.9,grayscale=True,region=(23+offset[0], 149+offset[1], 289+offset[0], 244+offset[1])):
        time.sleep(0.5)
    fd.click(154+offset[0], 288+offset[1])
    fd.wait_for_spawn(offset,0)
    if Do_AutoPos:
        fd.auto_positioner("Saber",offset)
        time.sleep(1)
        fd.click(67+offset[0],90+offset[1],right_click=True)
        time.sleep(3)
    while True:
        match_end = False
        win = False
