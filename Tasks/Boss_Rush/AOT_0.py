...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Boss_Rush", "AOT.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("AOT",offset)
        time.sleep(1)
        fd.click(802+offset[0],274+offset[1],right_click=True)
        time.sleep(3)
        fd.click(729+offset[0],285+offset[1],right_click=True)
    while True:
        match_end = False
        win = False
        for func in Order:
