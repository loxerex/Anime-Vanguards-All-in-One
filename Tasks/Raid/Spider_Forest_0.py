...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Raid", "Spider_Forest.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    if Do_AutoPos:
        fd.auto_positioner("Spider_Forest",offset)
        time.sleep(1)
    while True:
        match_end = False
        win = False
