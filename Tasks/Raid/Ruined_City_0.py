...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Raid", "Ruined_City.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def order_interpator(key,unit,action_index, other: bool | None = None):
    ...
    match key:
        ...
        case "cid_number":
            ...
            n = 3
            while n > 0:
                while not pyautogui.pixelMatchesColor(411+offset[0], 418+offset[1],expectedRGBColor=(255,10,17),tolerance=15):
                    if fd.does_exist("Victory.png",confidence=0.9,grayscale=True,region=(147+offset[0], 150+offset[1], 226+offset[0], 175+offset[1])) or fd.does_exist("Failed.png",confidence=0.9,grayscale=True,region=(147+offset[0], 150+offset[1], 226+offset[0], 175+offset[1])):
                        return
                    time.sleep(0.1)
                print("found num")
                ...
                while not got_num:
                    cur_num = -1 
                    for i in range (1,5):#(280, 438), (298, 449)
                        if fd.does_exist(f"Cid_Raid\\{i}.png",confidence=0.96,grayscale=True,region=(280+offset[0],438+offset[1],298+offset[0],449+offset[1])):
                            print(i)
                            cur_num = i
                    if cur_num != -1:
                    ...
                    n-=1
                if n!=0:
                    while pyautogui.pixelMatchesColor(411+offset[0], 418+offset[1],expectedRGBColor=(255,10,17),tolerance=15):
                            time.sleep(0.1)
            numb = [1,2,3,4]
...
def main():
    ...
    if Do_AutoPos:
        ...
        time.sleep(1)
        top_down_unit = Settings["top_down_unit"]
        fd.start(offset)
            ...
            if func[0] == "ability" and "AINZ_SPELLS" in Units.get(str(func[1])).get("abilities")[func[2]][1]:
                ...
                if "end" not in func[0]:
                    if "FT_" in func[0]:    
                        ...
                        if "cid_numb" in func[0] and not cid_numb1:
                            order_interpator(func[0],func[1],func[2])
                            cid_numb1 = True
                        elif "cid_numb" in func[0] and cid_numb1:
                            order_interpator(func[0],func[1],func[2],True)
                        else:
                            order_interpator(func[0],func[1],func[2])
                else:
                    match_end = True
