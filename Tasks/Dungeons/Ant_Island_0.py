...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Dungeons", "Ant_Island.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
card_priority = load_aio_settings()["Ant_Island"]["AI_Card_Priority"]

def cards_while_wait():
    while True:
        if Card_event.is_set():
            print("set")
            return
        if fd.does_exist(f"Ant\\Cards.png",confidence=0.8,grayscale=True,region=(360+offset[0], 139+offset[1], 456+offset[0], 176+offset[1])):
                fd.click(752+offset[0], 150+offset[1])
                time.sleep(1)
                for card in card_priority:
                    if fd.does_exist(f"Ant_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                        fd.click_image(f"Ant_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1]),offset=(0,0))
                        break
        time.sleep(2)
...
def order_interpator(key,unit,action_index):
    ...
    match key:
        ...
        case "wfw":
            ...
            do_cards = Thread(target=cards_while_wait,daemon=True)
            do_cards.start()
...
def main():
    First_Match = True
    AINZ_SPELL = False
    card_find = load_aio_settings()["Ant_Island"]["Ant_Island_Card"]
    ...
    if Do_AutoPos:
        while not is_pos:
            fd.auto_positioner("Ant_Island",offset,no_restart=True)
            time.sleep(1)
            fd.click(169+offset[0],553+offset[1],right_click=True)
            time.sleep(6)#(20, 82), (237, 246)
            fd.click(409+offset[0], 309+offset[1])
            if fd.does_exist("Ant\\Is_Position.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                is_pos = True
                print("In position.")
    fd.start(offset)
    time.sleep(6)
    ...
    while True:
        ...
        while not selected_card:
            if fd.does_exist(f"Ant\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                selected_card = True
                fd.click_image(f"Ant\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1]),offset=(0,0))
            else:
                fd.click(409+offset[0], 309+offset[1])
                fd.start(offset)
                time.sleep(6)
        ...
        win = False
        for func in Order:
            if fd.does_exist("Ant\\Cards.png",confidence=0.8,grayscale=True,region=(360+offset[0], 139+offset[1], 456+offset[0], 176+offset[1])):#(360, 139), (456, 176)
                fd.click(752+offset[0], 150+offset[1])
                time.sleep(1)
                for card in card_priority:
                    if fd.does_exist(f"Ant_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
            ...
            if func[0] == "ability" and "AINZ_SPELLS" in Units.get(str(func[1])).get("abilities")[func[2]][1]:
                ...
                if "end" not in func[0]:
                    ...
                    win = True
        while not match_end:
            if fd.does_exist("Ant\\Cards.png",confidence=0.8,grayscale=True,region=(360+offset[0], 139+offset[1], 456+offset[0], 176+offset[1])):
                fd.click(752+offset[0], 150+offset[1])
                time.sleep(1)
                for card in card_priority:
                    if fd.does_exist(f"Ant_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
