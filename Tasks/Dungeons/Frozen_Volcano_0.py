...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Dungeons", "Frozen_Volcano.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def main():
    ...
    card_find = load_aio_settings()["Frozen_Volcano"]["Frozen_Volcano_Card"]
    card_priority = load_aio_settings()["Frozen_Volcano"]["FV_Card_Priority"]
    
    fd.wait_for_spawn(offset,0)
    time.sleep(1)
    fd.click(409+offset[0], 309+offset[1])
    if Do_AutoPos:
        fd.auto_positioner("Frozen_Volcano",offset,just_camera=True,no_restart=True)
        time.sleep(1)
        fd.click(382+offset[0],623+offset[1],right_click=True)
        time.sleep(4)
        fd.click(382+offset[0],623+offset[1],right_click=True)
        time.sleep(2)
        fd.click(643+offset[0],619+offset[1],right_click=True)
    fd.start(offset)
    time.sleep(6)
    ...
    while True:
        ...
        while not selected_card:
            if fd.does_exist(f"Frozen\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                selected_card = True
                fd.click_image(f"Frozen\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1]),offset=(0,0))
            else:
                fd.click(409+offset[0], 309+offset[1])
                fd.start(offset)
                time.sleep(6)
        ...
        win = False
        for func in Order:
            if fd.get_wave(offset=offset) == -1:
                fd.click(752+offset[0], 150+offset[1])
                time.sleep(2)
                for card in card_priority:
                    if fd.does_exist(f"Frozen_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                        fd.click_image(f"Frozen_Cards\\{card}.png",confidence=0.8,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1]),offset=(0,0))
                        break
            if fd.does_exist("Frozen\\Save_Brolzi.png",confidence=0.9,grayscale=True,region=(524+offset[0], 491+offset[1], 653+offset[0], 520+offset[1])):
                fd.click_image("Frozen\\Save_Brolzi.png",confidence=0.9,grayscale=True,region=(524+offset[0], 491+offset[1], 653+offset[0], 520+offset[1]),offset=(0,0))
            # Win/Lose Check
