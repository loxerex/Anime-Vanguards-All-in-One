...
def load_settings():
    json_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), "Settings", "Dungeons", "Bleach_Dungeon.json")
    with open(json_path, 'r') as file:
        return json.load(file)   
...
def card_check():
    print("Running card check")
    card_priority = load_aio_settings()["Bleach_Dungeon"]["Bleach_Card_Priority"]
    while True:
        if is_match:
...
def load_aio_settings():
    ...
    with open(json_path, 'r') as file:
        return json.load(file)   
Thread(target=card_check,daemon=True).start()
def main():
    First_Match = True
    AINZ_SPELL = False
    global is_match
    card_find = load_aio_settings()["Bleach_Dungeon"]["Bleach_Card"]
    card_priority = load_aio_settings()["Bleach_Dungeon"]["Bleach_Card_Priority"]
    ...
    if Do_AutoPos:
        fd.auto_positioner("no",offset,just_camera=True,no_restart=True)
        #[(786, 204), (767, 347)]
        fd.click(786+offset[0],204+offset[1],right_click=True)
        time.sleep(3.5)
        fd.click(767+offset[0],347+offset[1],right_click=True)
    fd.start(offset)
    time.sleep(6)
    ...
    while True:
        ...
        while not selected_card:
            if fd.does_exist(f"BleachDungeon\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1])):
                selected_card = True
                fd.click_image(f"BleachDungeon\\{card_find}.png",confidence=0.9,grayscale=True,region=(42+offset[0], 180+offset[1], 781+offset[0], 464+offset[1]),offset=(0,0))
            else:
                fd.click(409+offset[0], 309+offset[1])
                fd.start(offset)
                time.sleep(6)
